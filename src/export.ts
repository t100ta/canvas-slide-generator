...
export const exportAsPNG = async (
  ...
  const { canvas } = renderCtx;

  // Export current slide as PNG
  const link = document.createElement("a");
  link.download = `slide-${renderCtx.navigation.currentSlide + 1}.png`;
...
export const exportAllSlidesAsPNG = async (
  ...
  // This would require rendering each slide individually
  // For now, just export the current slide
  await exportAsPNG(renderCtx, _content);
};
...
export const exportAsPDF = async (
  ...
  const { config } = renderCtx;

  // Create PDF document in landscape mode to match 16:9 aspect ratio
  const pdf = new jsPDF({
  ...
  // Store current slide
  const originalSlide = renderCtx.navigation.currentSlide;
  ...
  try {
    ...
    pdf.save(`canvas-slides-${timestamp}.pdf`);
  } finally {
    // Restore original slide
    renderCtx.navigation.currentSlide = originalSlide;
  }
};
